...
struct WebViewContainer: UIViewRepresentable {
  ...
  @StateObject var volumer: Volumer
  ...
  func updateUIView (_ uiView: WKWebView, context: Context){
    // 在变量变化时执行相应的操作
    uiView.evaluateJavaScript("setVolume(\(volumer.volume))")
  }
  ...
  class Coordinator: NSObject, WKScriptMessageHandler, WKNavigationDelegate {
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
      if message.name == "volumeChanged", let volumeInfo = message.body as? [String: Any], let volumeNow = volumeInfo["volume"] as? Double {
        // 在此处理收到的音量信息
        self.parent.volumer.showTip = true
        self.parent.volumer.volume = Float(volumeNow)
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
          self.parent.volumer.showTip = false
        }
      }
    }
...
struct VolumeControlView: UIViewRepresentable {
  ...
  func makeUIView(context: Context) -> MPVolumeView {
    ...
    volumeView.showsVolumeSlider = volumer.showTip
    // 找到进度条的子视图，并设置进度条颜色
  ...
  func setVolume(_ uiView: MPVolumeView) {
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.01) {//延迟0.01秒就能够正常播放
      volumeSlider?.value = volumer.volume
    }
  }
}
...
struct ContentView: View {
  ...
  var body: some View {
    ZStack {
      VStack {
        if global.serverBooted {
          WebViewContainer(
            global: global,
            volumer: volumer,
            configuration: createConfiguration(volume: $volumer.volume),
      ...
      if volumer.showTip {
        VolumeControlView(volumer: volumer )
          .frame(width: 200)
          .position(x: -200, y: -200)
          .allowsHitTesting(false)
        
        VStack {
          Text("音量: \(Int(volumer.volume*100))")
            .padding(.vertical, 8)
